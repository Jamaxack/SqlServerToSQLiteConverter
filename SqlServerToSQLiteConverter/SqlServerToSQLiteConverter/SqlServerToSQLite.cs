...
namespace SqlServerToSQLiteConverter
{
    ...
    public class SqlServerToSQLite
    {
        ...
        /// immediatly.</remarks>
        public static void Process(string sqlServerConnString, string sqlitePath, string password, bool createTriggers, bool createViews, List<string> excludeTables)
        {
            ...
            {
                _isActive = true;
                ConvertSqlServerDatabaseToSQLiteFile(sqlServerConnString, sqlitePath, password, createTriggers, createViews, excludeTables);
                _isActive = false;
            }
            catch (Exception ex)
            {
                _log.Error(ex, "Failed to convert SQL Server database to SQLite database");
                _isActive = false;
            } // catch
        }
        #endregion
        ...
        /// <param name="password">The password to use or NULL if no password should be used to encrypt the DB</param> 
        private static void ConvertSqlServerDatabaseToSQLiteFile(string sqlConnString, string sqlitePath, string password, bool createTriggers, bool createViews, List<string> excludeTables)
        {
            ...
            DatabaseSchema ds = ReadSqlServerSchema(sqlConnString, excludeTables);
            ...
            CopySqlServerRowsToSQLiteDB(sqlConnString, sqlitePath, ds.Tables, password);
        ...
        private static void CopySqlServerRowsToSQLiteDB(string sqlConnString, string sqlitePath, List<TableSchema> schema, string password)
        {
            CheckCancelled();
            ...
            using (SqlConnection ssconn = new SqlConnection(sqlConnString))
            {
                ...
                using (SQLiteConnection sqconn = new SQLiteConnection(sqliteConnString))
                {
                    ...
                    for (int i = 0; i < schema.Count; i++)
                    {
                        ...
                        {
                            _log.Error(ex, "unexpected exception");
                            tx.Rollback();
                            throw;
                        } // catch
                    }
        ...
        private static void CreateSQLiteDatabase(string sqlitePath, DatabaseSchema schema, string password, bool createViews)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(sqliteConnString))
            {
                ...
                foreach (TableSchema dt in schema.Tables)
                {
                    ...
                    {
                        _log.Error(ex, "AddSQLiteTable failed");
                        throw;
                    }
                    count++;
                    CheckCancelled();
                ...
                if (createViews)
                {
                    foreach (ViewSchema vs in schema.Views)
                    {
                        ...
                        {
                            _log.Error(ex, "AddSQLiteView failed");
                            throw;
                        } // catch
                        count++;
                        CheckCancelled();
        ...
        private static DatabaseSchema ReadSqlServerSchema(string connString, List<string> excludeTables)
        {
            ...
            tables = tables.Where(t => !excludeTables.Any(et => et == t.TableName)).ToList();
            ...
            string password)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(sqliteConnString))
            {
                ...
                foreach (TableSchema dt in schema)
                {
                    ...
                    {
                        _log.Error(ex, "AddTableTriggers failed");
                        throw;
                    }
                }
